...
const BookingSchema = new Schema<IBooking>(
  {
    ...
    organizationOrSchool: {
      type: String,
      trim: true,
      maxlength: [100, 'Organization or school cannot exceed 100 characters'],
    },
  },
  {
  ...
  if (booking.isModified('eventId') || booking.isNew) {
    try {
      ...
      const validationError = new Error(
        `Invalid event ID format or database error: ${err instanceof Error ? err.message : String(err)}`
      );
